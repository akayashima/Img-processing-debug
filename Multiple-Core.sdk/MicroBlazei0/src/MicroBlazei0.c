...
int main()
{
	...
	myprintf(buff,"ABCDEFG\n");

	myprintf(buff,"HIJKLMN\n");
	//checkTimer();
...
int myprintf(char* buff,char* fmt, ...){
	va_list arg;
	int len;
	int size;
	int zeroflag,width;
...
int vmyprintf(char* buff,char* fmt,va_list arg){
	...
	while(*fmt){
		if(*fmt=='%'){		/* % に関する処理 */
			...
			switch(*fmt){
			case 'd':		/* 10進数 */
				size = myprintf_decimal(va_arg(arg,signed long),buff,zeroflag,width);
				break;
			case 'x':		/* 16進数 0-f */
				size = myprintf_hexadecimal(va_arg(arg,unsigned long),buff,0,zeroflag,width);
				break;
			case 'X':		/* 16進数 0-F */
				size = myprintf_hexadecimal(va_arg(arg,unsigned long),buff,1,zeroflag,width);
				break;
			case 'c':		/* キャラクター */
				size = myprintf_char(va_arg(arg,int),buff);
				break;
			case 's':		/* ASCIIZ文字列 */
				size = myprintf_string(va_arg(arg,char*),buff);
				break;
...
/*
  数値 => 10進文字列変換
*/
static int myprintf_decimal(signed long val,char* buff,int zf,int wd){
	int i;
	char tmp[10];
	char* ptmp = tmp + 10;
	int len = 0;
...
/*
  数値 => 16進文字列変換
*/
static int myprintf_hexadecimal(unsigned long val,char* buff,
...
/*
  数値 => 1文字キャラクタ変換
*/
static int myprintf_char(int ch,char* buff){
	*buff = (char)ch;
	return(1);
}
...
/*
  数値 => ASCIIZ文字列変換
*/
static int myprintf_string(char* str,char* buff){
	int count = 0;
	while(*str){
		*(buff++) = *str;
		str++;
